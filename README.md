...
DocuFlow is a containerized, event-driven data pipeline that automates ingestion, OCR, parsing, and analytics for invoices and other financial documents. It uses a small microservice stack (watcher -> broker -> workers -> database -> dashboard) so heavy OCR work runs asynchronously without blocking the UI.
...
```mermaid
graph LR
    A[📂 data/raw (Hot Folder)] -->|New PDF detected| B[🕵️ Watcher Service]
    B -->|Push Task| C{Redis (Broker)}
    C -->|Distribute Task| D[⚙️ Celery Worker(s)]
...
- The watcher uses polling (compatible with WSL2/Docker on Windows); renaming/moving in some OSes may not trigger events—see troubleshooting.

---

## 🔧 Configuration & Environment

The project is expected to use environment variables for DB and broker configuration inside docker-compose. If you extend or run services manually, ensure the following (example names):
...
(If you want, I can add a sample `.env.example` file with recommended settings.)
